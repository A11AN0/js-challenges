...
export const accessGivenKey = (object, key) => {
  ...
  const keyArrayNumber = keysArr.indexOf(key);
  const objectValuesArray = Object.values(object); 

  return objectValuesArray[keyArrayNumber];
}; //lol kinda went the long way with this
...
export const getUserAddress = (user) => {
  ...
  return(`${addressObj.line1} ${addressObj.line2} ${addressObj.city} ${addressObj.postcode}`);
};
...
export const setSafeAllergens = (customer, allergenList) => {
  ...
  const safeAllergens =Array.from(allergenList);

  for(var i=0; i<allergenList.length; i++){
    const allergen = allergenList[i]
 
    for(var m=0; m<allergies.length; m++){
        const allergy = allergies[m]
        if(allergy === allergen){
         safeAllergens.splice(safeAllergens.indexOf(allergy),1);
        }
...
export const mergeFurniture = (furnitureLocationData, furnitureProductData) => {
  const mergedFurnitureData = Object.assign(furnitureLocationData,furnitureProductData);
  return mergedFurnitureData;

  //need to make a loop that goes thru the values for each, and if either is equal, then do something? idk lol
};
